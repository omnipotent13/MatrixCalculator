...
MATRIX InputMatrix(char op, int m, int n)
{
	...
	for (i = 0; i < x.m; i++)
	{
		for (j = 0; j < x.n; j++)
		{
		INPUTELEMENTS:judge = scanf_s("%f", x.a + i * x.n + j);	//输入矩阵元素值。
			if (judge == 0)
			{
				rewind(stdin);
